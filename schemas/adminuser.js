...
var UsersModel = mongoose.model("adminuser", UsersSchema);
UsersModel.findOne({'username':'admin'}, function(err, res){
    if (err) {
        console.error(err);
    }
    if(!res){
        var userModel = new UsersModel({ username:"admin",password: 'admin2018'});
